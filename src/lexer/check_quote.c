...
/*   Updated: 2023/07/26 13:41:12 by nibernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

static void	get_string_double_quote(t_data *data, int j, int len, char quote)
{
	t_lexer	*tmp;
	char	*str;
	int		i;
	...
	str[i] = '\0';
	i = 0;
	tmp = ft_lexer_new(str, WORD, DOUBLE);
	if (!tmp)
	{
		free(str);
...
static void	get_string_single_quote(t_data *data, int j, int len, char quote)
{
	t_lexer	*tmp;
	char	*str;
	int		i;
...
int	check_quote(t_data *data, int i)
{
	...
	if (data->input[i] == quote && quote == 34)
	{
		...
		if (check_quote == 0)
			get_string_double_quote(data, (j - 1), (i - j), quote);
		return (i + 1);
	}
	else if (data->input[i] == quote && quote == 39)
		get_string_single_quote(data, (j - 1), (i - j), quote);
	else
		data->quote_error = 1;
	return (i + 1);
}
